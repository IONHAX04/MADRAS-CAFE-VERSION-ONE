  ...
  return (
    <div className="bg-white">
      ...
      <section className="max-w-5xl mx-auto py-16 px-6 grid grid-cols-1 md:grid-cols-3 gap-6 paper-texture">
        {[
          { icon: Instagram, name: '@MADRASCAFE', url: 'https://instagram.com/madrascafe', color: '#E1306C' },
          { icon: Facebook, name: '/MADRASCAFE', url: 'https://facebook.com', color: '#1877F2' },
          { icon: Heart, name: 'PINTEREST', url: 'https://pinterest.com', color: '#E60023' },
        ].map((s) => (
          <a key={s.name} href={s.url} target="_blank" rel="noreferrer" className="group bg-white border-2 border-neutral-200 hover:border-[#1a5e3a] p-8 text-center transition-colors">
            <s.icon size={36} className="mx-auto text-[#1a5e3a] group-hover:scale-110 transition-transform" />
      ...
      <section className="max-w-7xl mx-auto pb-24 px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {INSTAGRAM_FEED.map((src, i) => (
            <a key={i} href="https://instagram.com/madrascafe" target="_blank" rel="noreferrer" className="group block aspect-square bg-neutral-200 overflow-hidden">
              <img src={src} alt="Madras Cafe social post" loading="lazy" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
            </a>
          ))}
        </div>
      </section>
